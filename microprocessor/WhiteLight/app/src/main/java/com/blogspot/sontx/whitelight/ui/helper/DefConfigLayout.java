 ...
 */
public class DefConfigLayout extends ConfigLayout implements AdapterView.OnItemSelectedListener,
    ...
    public DefConfigLayout(DefConfig defConfig, UserConfig userConfig, int lightId) {
    ...
    public void setDefConfig(DefConfig defConfig) {
        this.defConfig = defConfig;

        String[] lightTypes = context.getResources().getStringArray(R.array.light_types);
        tvLightType.setText(String.format("Apply default config for %s", lightTypes[userConfig.getLightType() - 1]));
    ...
    private void cacheChanged(int level, int typeId) {
        ...
        if (typeId == 7) {
            ...
            DefConfig.Level[] levels = defConfig.exportState();
            DefConfig.Level _level = new DefConfig.Level(typeId, false);
            levels[level] = _level;
    ...
    public void onClick(View v) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                boolean ok = ServerConnection.getInstance().updateDefConfig(defConfig, lightId);
