package com.blogspot.sontx.whitelight.lib;

/**
 * Copyright by NE 2015.
 * Created by noem on 23/11/2015.
 */
public final class Config {
    public static final String SHARED_USERCONFIG = "shared_userconfig";
    public static final String SHARED_DEFCONFIG = "shared_defconfig";
    public static final String SHARED_LIGHTS = "shared_lights";
    public static final String SHARED_REFRESH_RATE = "refresh_rate";
    public static final String SHARED_IPADDR = "ip_addr";
    public static final String SHARED_PORTNUM = "port_numb";
    public static final int MAX_SUPPORT_LIGHTS = 10;
}
